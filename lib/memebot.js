...
function MemeBot(client) {
  this.client = client;
  this.channel = client.opt.channels[0];
...
//
MemeBot.prototype.say = function(msg) {
  this.client.say(this.channel, msg.toLowerCase().replace('\n', ''));
};
...
MemeBot.prototype.handlePm = function(from) {
  ...
  automeme.getMeme()
    .then(function(meme) {
      client.say(from, meme.toLowerCase().replace('\n', ''));
    })
    .fail(console.error.bind(this, 'Failed to get meme:'))
    .done();
};
